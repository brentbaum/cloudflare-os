...
describe("credential wrapping", () => {
  it("round-trips with authenticated object and purpose binding", async () => {
    ...
    await expect(keyring.decrypt(envelope, "object-fake-1", "credential")).resolves.toEqual({
      accessToken: "access_unmistakably_fake",
      refreshToken: "refresh_unmistakably_fake",
    });
    await expect(keyring.decrypt(envelope, "object-fake-2", "credential")).rejects.toThrow(
      "could not be authenticated",
    );
    await expect(keyring.decrypt(envelope, "object-fake-1", "pending")).rejects.toThrow(
      "could not be authenticated",
    );
  });
  ...
  it("decrypts with the previous key but always encrypts with the current key", async () => {
    ...
    await expect(rotated.decrypt(oldEnvelope, "object-fake", "credential")).resolves.toEqual({
      value: "fake-old",
    });
    const newEnvelope = await rotated.encrypt({ value: "fake-new" }, "object-fake", "credential");
  ...
  it("rejects tampering and unavailable key ids", async () => {
    ...
    const envelope = await keyring.encrypt({ value: "fake" }, "object-fake", "credential");
    await expect(
      keyring.decrypt(
    ...
    await expect(
      WrappingKeyring.create(fakeKey(5)).then((other) =>
        other.decrypt(envelope, "object-fake", "credential"),
      ),
