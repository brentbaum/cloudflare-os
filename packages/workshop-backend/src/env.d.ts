...
// A service binding carries the frozen RPC interface, but Workers' generated `Service<T>` generic
// accepts only a concrete branded WorkerEntrypoint class. The relay class lives in an independently
// merged package, so this structural intersection keeps this lane typed without coupling to it.
...
declare global {
  namespace Cloudflare {
    interface Env {
      ...
      // Private shared-subscription inference. Both the explicit flag and binding are required;
      // leaving either absent keeps the feature inert.
      CODEX_SUBSCRIPTION_ENABLED?: string;
      CODEX_RELAY?: CodexRelayService;
